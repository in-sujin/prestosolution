...
def get_data(dataset, max_train_size=None, max_test_size=None,
    ...
    f = open(os.path.join(prefix, "train.pkl"), "rb")
    train_data = np.array(pickle.load(f)).reshape((-1, x_dim))[train_start:train_end, :]
    f.close()
    
    try:
        f = open(os.path.join(prefix, "test.pkl"), "rb")
    ...
    try:
        f = open(os.path.join(prefix, "test_label.pkl"), "rb")
